...
def split_videos(video_folder, video_list_file, clip_size):
    # Process args
    ...
    for video in videos:
        ...
        num_clips = floor(total_frames / clip_size)
        ...
        while success:
            ...
            if len(frames) == clip_size:
                ...
                out = cv2.VideoWriter(folder_name + '/' + str(clip_num) + '.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 25,
                                      (cols, rows))
                for frame in frames:
                    out.write(frame)
                out.release()
        ...
        if len(frames) != 0:
            # Repeat the last frame the necessary ti,es
...
if __name__ == "__main__":
    ...
    split_videos(sys.argv[1], sys.argv[2], sys.argv[3])
