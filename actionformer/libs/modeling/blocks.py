...
# TODO find the line where the features are downsampled
# The first few layers do not downsample, there must be an option to turn it on/off
class LocalMaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        n_qx_stride=1,   # dowsampling stride for query and input
        n_kv_stride=1,   # downsampling stride for key and value
        # If both strides equal 1, there is no downsampling
        # Will need to modify them to get an upsampling, or add another param
        attn_pdrop=0.0,  # dropout rate for the attention map
        proj_pdrop=0.0,  # dropout rate for projection op
        use_rel_pe=False, # use relative position encoding
        upsampling=False # If true, ignores downsampling and executes a temporal upsampling (x2 temporal resolution)
    ):
        super().__init__()
        # If we are doing an upsampling of the temporal resolution there must be no downsampling
        assert not upsampling or n_qx_stride == n_kv_stride == 1
    ...
    def forward(self, x, mask):
        ...
        # Apply upsampling to the output and the mask when needed
        if self.upsampling:
            self.upsample_layer = nn.Upsample(scale_factor=2, mode='nearest')
            out = self.upsample_layer(out)
            qx_mask = self.upsample_layer(qx_mask.float())
            qx_mask = qx_mask.bool()
            return out, qx_mask

        return out, qx_mask


# TODO create a similar block, using upsampling at the end instead of downsampling
# Basic Transformer Block, with downsampling added
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        upsampling=False,      # true if we want a temporal upsampling
    ):
        super().__init__()
...
# TemporalMaxer block, similar to the TransformerBlock, using a MaxPooling operation instead of MaskedMHA
# Code from: https://github.com/TuanTNG/TemporalMaxer/blob/master/libs/modeling/blocks.py
