...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.ground_truth.to_csv('ground_truth.csv')
    ...
    def evaluate(self, preds, verbose=True):
        ...
        unique_labels = np.unique(preds['label'].tolist())  # Get unique labels in the prediction
        preds.to_csv('preds.csv')
        preds['label'] = preds['label'].replace(self.activity_index)
        
        
        # Save an activity index for the prediction classes, so we know which ones are in the ground truth
        """preds_activity_index = dict()
        for label in unique_labels:
            if label in self.activity_index.keys():
                preds_activity_index.update({str(label): int(self.activity_index[label])})
            else:
                preds_activity_index.update({str(label): int(label)})
                  
        # Add the ground truth activity indexes to the preds one if they are not already present
        for label in self.activity_index.keys():
            if label not in preds_activity_index.keys():
                preds_activity_index.update({str(label): int(self.activity_index[label])})
                
        print(preds_activity_index)"""
        ...
        # Save activity index to file, so we know the original activity class ids
        """print("Activity index: \n", self.activity_index)
        activity_file = open("activity_index.json", "w+")
        json.dump(preds_activity_index, activity_file)
        activity_file.close()"""
